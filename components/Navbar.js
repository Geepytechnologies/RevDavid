...
export default function Navbar(props) {
    ...
    const close2 = useRef();
    const show = () => {
        console.log('transshow');
        document.body.style.overflow = 'hidden';
        menu2.current.style.display = 'none';
        close2.current.style.display = 'flex';
        ...
        solidnav.current.style.backgroundColor ='#9b0000';
    }
    const show2 = () => {
        console.log('solidshow');
        document.body.style.overflow = 'hidden';
        menu2.current.style.display = 'none';
        close2.current.style.display = 'flex';
        side2.current.style.maxHeight = 'calc(100vh - 10px)';
        side2.current.style.borderBottom = '1px solid rgba(255, 255, 255, 0.3)';
    }
    const hide = () => {
        console.log('transclose');
        document.body.style.overflow = 'scroll';
        menu.current.style.display = 'flex';
        close.current.style.display = 'none';
        side2.current.style.maxHeight = '0px';
        side2.current.style.borderBottom = '0px';
        side2.current.style.borderTop = '0px';
        solidnav.current.style.display ='none';

    }
    const hide2 = () => {
        console.log('solidclose');
        document.body.style.overflow = 'scroll';
        menu2.current.style.display = 'flex';
    ...
    <div className='relative'>
      <div ref={transnav} className='text-white w-[100%]  h-[110px] flex items-center absolute top-0 z-[100] '>
            ...
            <div onClick={()=>{router.push('/')}} className='flex items-center basis-[70%] sm:basis-[50%] md:basis-[30%] lg:[25%]'>
                <p className='font-allura pl-[5px] font-[700] text-[20px] sm:text-[30px] text-[#9b0000]'>Rev David Akanwa</p>
            </div>
            {/* end of logo section */}
            {/* menu section */}
